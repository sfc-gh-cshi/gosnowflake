...
func getOIDFromHashAlgorithm(target crypto.Hash) asn1.ObjectIdentifier {
	...
	logger.Infof("no valid OID is found for the hash algorithm. %#v", target)
	return nil
}
...
func getHashAlgorithmFromOID(target pkix.AlgorithmIdentifier) crypto.Hash {
	...
	logger.Infof("no valid hash algorithm is found for the oid. Falling back to SHA1: %#v", target)
	return crypto.SHA1
}
...
func checkOCSPCacheServer(
	...
	if err != nil {
		logger.Infof("failed to get OCSP cache from OCSP Cache Server. %v\n", err)
		return nil, &ocspStatus{
	...
	for {
		if err := dec.Decode(&respd); err == io.EOF {
			...
			logger.Infof("failed to decode OCSP cache. %v\n", err)
			return nil, &ocspStatus{
...
func validateWithCache(subject, issuer *x509.Certificate) (*ocspStatus, []byte, *certIDKey) {
	...
	if err != nil {
		logger.Infof("failed to create OCSP request from the certificates.\n")
		return &ocspStatus{
	...
	if ocspS.code != ocspSuccess {
		logger.Infof("failed to extract CertID from OCSP Request.\n")
		return &ocspStatus{
			code: ocspFailedComposeRequest,
...
func writeOCSPCacheFile() {
	...
	case os.IsExist(err):
		...
		if err != nil {
			logger.Infof("failed to write OCSP response cache file. file: %v, err: %v. ignored.\n", cacheFileName, err)
			return
		}
		if time.Since(statinfo.ModTime()) < 15*time.Minute {
			logger.Infof("other process locks the cache file. %v. ignored.\n", cacheFileName)
			return
		}
		err = os.Remove(cacheLockFileName)
		if err != nil {
			logger.Infof("failed to delete lock file. file: %v, err: %v. ignored.\n", cacheLockFileName, err)
			return
		}
		err = os.Mkdir(cacheLockFileName, 0600)
		if err != nil {
	...
	if err != nil {
		logger.Info("failed to convert OCSP Response cache to JSON. ignored.")
		return
	}
	err = ioutil.WriteFile(cacheFileName, j, 0644)
	if err != nil {
		logger.Infof("failed to write OCSP Response cache. err: %v. ignored.\n", err)
	}
}
...
func createOCSPCacheDir() {
	...
	if _, err := os.Stat(cacheDir); os.IsNotExist(err) {
		...
		if err != nil {
			logger.Infof("failed to create cache directory. %v, err: %v. ignored\n", cacheDir, err)
		}
	}
